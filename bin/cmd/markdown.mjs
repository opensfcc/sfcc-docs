...
const debug = Debug('sfcc-docs:markdown')
          ...
          if (fs.existsSync(file)) {
              ...
              turndownService.addRule('param-title', {
                  filter: (node, options) => {
                      return (
                        node.nodeName === 'SPAN' &&
                        node.classList.contains('parameterTitle')
                      )
                  },
                  replacement: (content) => {
                    return '`' + content + '`'
              ...
              turndownService.addRule('param-details', {
                  filter: (node, options) => {
                      return (
                        node.nodeName === 'DIV' &&
                        node.classList.contains('parameterDetail')
                      )
                  },
                  replacement: (content) => {
                    return ' _' + content + '_\n'
